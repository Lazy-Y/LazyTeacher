...
class CVCalendarMonthContentViewController: CVCalendarContentViewController {
    ...
    override func togglePresentedDate(date: NSDate) {
        print("here  \(date.description)")
        let presentedDate = Date(date: date)
        var calDate:CVDate!
        if calendarView.coordinator.selectedDayView?.date == nil{
            calDate = CVDate(date: monthViews[Presented]!.date)
        }
        else {
            calDate = calendarView.coordinator.selectedDayView?.date
        }
        if let presented = monthViews[Presented], let selectedDate = calDate {
